...
z = 1/np.exp(x) - 1
plt.figure()
plt.plot(z, background["d_lumin"]/z, color = "grey", label = "Luminosity distance")
plt.errorbar(supernova["z"], supernova["d_L (Gpc)"]/supernova["z"], yerr = supernova["Error (Gpc)"], fmt = ".", color = "k", label = "Supernova data")
#plt.yscale("log")
#plt.xscale("log")
#plt.xlim(right = 2)
plt.xlim([-0, 1.5])
